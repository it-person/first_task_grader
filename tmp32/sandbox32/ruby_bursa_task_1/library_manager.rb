 class LibraryManager
  ...
  def penalty price, issue_datetime
    ...
    date1 = issue_datetime
    date2 = DateTime.now.new_offset(0)
    diff_in_hours = (date2.to_i - date1.to_i) / 3600
    return 0 if price < 0 || diff_in_hours < 0
    #binding.pry 
    penalty = (0.001 * price * diff_in_hours).round
  ...
  def could_meet_each_other? year_of_birth_first, year_of_death_first, year_of_birth_second, year_of_death_second
    ...
    #(year_of_birth_first..year_of_death_first).include?(year_of_birth_second..year_of_death_second)
    !(year_of_birth_first..year_of_death_first).select do |n| 
      (year_of_birth_second..year_of_death_second).include?(n)
    end.empty?
  end
  ...
  def days_to_buy price
    ...
    daily_penalty = 0.001 * price * 24
    days_to_buy = (price / daily_penalty).to_i  
  ...
  def penalty_to_finish price, issue_datetime, pages_quantity, current_page, reading_speed
    ...
    pages_to_finish = pages_quantity - current_page
    time_to_finish = reading_speed * pages_to_finish
    lim_time = (issue_datetime.to_i - DateTime.now.new_offset(0).to_i - time_to_finish.to_i) / 3600

    if lim_time >= 0
      penalty_to_finish = 0
    else 
      penalty_to_finish = (0.001 * price * (-1 * lim_time)).round
    end
