class LibraryManager
  ...
  def penalty price, issue_datetime
    # решение пишем тут
    new_date = DateTime.now.new_offset(0)
    time = (new_date.to_time - issue_datetime.to_time).to_i / 3600
    (price * 0.001 * time).round
    # binding.pry
  ...
  def could_meet_each_other? year_of_birth_first, year_of_death_first, year_of_birth_second, year_of_death_second
    ...
    birth1 = 2015 - year_of_birth_first
    death1 = 2015 - year_of_death_first

    birth2 = 2015 - year_of_birth_second
    death2 = 2015 - year_of_death_second

    rez = (death1..birth1).overlaps?(death2..birth2)
  ...
  def days_to_buy price
    # решение пишем тут
    only_day = (price * 0.001) * 24

    day = (price / only_day).round
  ...
  def penalty_to_finish price, issue_datetime, pages_quantity, current_page, reading_speed
    ...
    # решение НЕ РЕАЛИЗОВАНО - В ПРОЦЕССЕ  )))))
    # не успела

    new_date = DateTime.now.new_offset(0)
    time_vozvrat = (new_date.to_time - issue_datetime.to_time).to_i / 3600
    ostatok = (pages_quantity - current_page) + 1
    time_read = ostatok / reading_speed

    t1 = time_vozvrat - time_read

    (price * 0.001 * t1).round
